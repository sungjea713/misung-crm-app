...
interface WeeklyPlanTableProps {
  ...
  showOnlyActivities?: boolean;
  showOnlyTargets?: boolean;
}

export function WeeklyPlanTable({ plans, onEdit, pagination, onPageChange, showOnlyActivities, showOnlyTargets }: WeeklyPlanTableProps) {
    ...
    return (
      <div className="card">
        <div className="flex flex-col items-center justify-center py-20 text-gray-text">
          <p className="text-lg mb-2">등록된 계획이 없습니다.</p>
  ...
  if (showOnlyActivities) {
    // Filter for activity plans only
    const activityPlans = plans.filter(plan =>
      plan.plan_type === 'activity' || plan.plan_type === 'both'
    );
  ...
  // 목표 금액 계획 테이블
  if (showOnlyTargets) {
    // Filter for target plans only
    const targetPlans = plans.filter(plan =>
      plan.plan_type === 'target' || plan.plan_type === 'both'
    );
      ...
      <div className="space-y-4">
        <div className="card overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-border">
                <th className="px-4 py-3 text-right text-sm font-semibold text-gray-text">목표 매출</th>
                <th className="px-4 py-3 text-right text-sm font-semibold text-gray-text">목표 수주 (매출 기여)</th>
                <th className="px-4 py-3 text-right text-sm font-semibold text-gray-text">목표 수주 (이익 기여)</th>
                <th className="px-4 py-3 text-right text-sm font-semibold text-gray-text">목표 수주 (합계)</th>
                <th className="px-4 py-3 text-right text-sm font-semibold text-gray-text">목표 수금</th>
                <th className="px-4 py-3 text-left text-sm font-semibold text-gray-text">생성날짜</th>
                <th className="px-4 py-3 text-left text-sm font-semibold text-gray-text">수정날짜</th>
                <th className="px-4 py-3 text-left text-sm font-semibold text-gray-text">액션</th>
              </tr>
              ...
              {targetPlans.map((plan) => (
                ...
                >
                  <td className="px-4 py-3 text-sm text-blue-400 text-right font-mono">
                    {formatNumber(plan.target_sales)}원
                  </td>
                  <td className="px-4 py-3 text-sm text-amber-400 text-right font-mono">
                    {formatNumber(plan.target_order_sales_contribution)}원
                  </td>
                  <td className="px-4 py-3 text-sm text-amber-400 text-right font-mono">
                    {formatNumber(plan.target_order_profit_contribution)}원
                  </td>
                  <td className="px-4 py-3 text-sm text-amber-500 text-right font-mono font-semibold">
                    {formatNumber(plan.target_order_total)}원
                  </td>
                  <td className="px-4 py-3 text-sm text-green-400 text-right font-mono">
                    {formatNumber(plan.target_collection)}원
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-text">
                    {formatDate(plan.created_at)}
                  </td>
