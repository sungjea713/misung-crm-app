...
interface DailyPlanFormProps {
  ...
  plan?: DailyPlan;
  onClose: () => void;
  onSave: (data: DailyPlanFormData) => Promise<void>;
  onDelete?: (id: number) => Promise<void>;
}

export function DailyPlanForm({ user, plan, onClose, onSave, onDelete }: DailyPlanFormProps) {
  const isEdit = !!plan;
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="page-title mb-1">
              {isEdit ? '일일 업무 일지 수정' : '일일 업무 일지 작성'}
            </h1>
