...
export default function MonthlyCollectionPage({ user }: MonthlyCollectionPageProps) {
  ...
  const handleDataParsed = async (parsedData: MonthlyCollectionRow[]) => {
  ...
  // 합계 계산
  const totalCollection = data.reduce((sum, item) => sum + item.collection_amount, 0);
