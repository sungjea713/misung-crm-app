...
export default function OutstandingBalance({ user }: OutstandingBalanceProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      // TODO: API 호출로 데이터 저장
      // const response = await fetch('/api/admin/outstanding-balance', {
      //   method: 'POST',
      //   headers: {
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-text">매월 미수금 누적 금액을 입력하세요.</p>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-border">
              ...
              {/* Last Month Value (as current total) */}
              <tr className="bg-bg-darker font-bold">
                <td className="px-6 py-4 whitespace-nowrap text-sm text-white">현재 미수금</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-red-500">
                  {formatNumber(monthlyData[monthlyData.length - 1]?.amount || 0)} 원
                </td>
              </tr>
            </tbody>
          </table>
