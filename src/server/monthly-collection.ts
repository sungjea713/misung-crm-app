...
// 월별 수금/미수금 데이터 조회
export async function getMonthlyCollection(year: number, month: number) {
  try {
...
// 월별 수금/미수금 데이터 저장/수정 (기존 데이터 삭제 후 새로 삽입)
export async function saveMonthlyCollection(data: MonthlyCollectionData, createdBy: string) {
  try {
    ...
    console.error('Error in saveMonthlyCollection:', error);
    return { success: false, message: error.message || '데이터 저장에 실패했습니다.' };
  }
}

// 월별 수금/미수금 데이터 삭제
export async function deleteMonthlyCollection(year: number, month: number) {
  try {
    ...
    if (error) {
      console.error('Error deleting monthly collection:', error);
      return { success: false, message: '데이터 삭제에 실패했습니다.' };
    }

    return { success: true, message: '데이터가 삭제되었습니다.' };
  } catch (error: any) {
    console.error('Error in deleteMonthlyCollection:', error);
